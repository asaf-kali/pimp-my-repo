...
class GitignoreBoost(Boost):
    ...
    def _detect_templates(self) -> list[str]:
        ...
        if any((self.tools.repo_path / f).exists() for f in ["pyproject.toml", "setup.py", "requirements.txt"]):
            templates.append("python")
        if (self.tools.repo_path / "package.json").exists():
            templates.append("node")
        if (self.tools.repo_path / "Cargo.toml").exists():
            templates.append("rust")
        if (self.tools.repo_path / "go.mod").exists():
            templates.append("go")
        if (self.tools.repo_path / "pom.xml").exists():
            templates.extend(["java", "maven"])
        if any((self.tools.repo_path / f).exists() for f in ["build.gradle", "build.gradle.kts"]):
    ...
    def _append_gitignore(self, generated: str) -> None:
        ...
        gitignore_path = self.tools.repo_path / ".gitignore"
